...
let _isAdmin       = false;
let _adminName     = '';
// Cloud sync config (saved in localStorage, never pushed to cloud)
let _cloudSettings = {
  rawUrl:   '',   // https://raw.githubusercontent.com/USER/REPO/main/aushadhi-data.json
  apiUrl:   '',   // https://api.github.com/repos/USER/REPO/contents/aushadhi-data.json
  pat:      '',   // GitHub Personal Access Token (admin only)
  fileSha:  ''    // SHA of last fetched file (needed for GitHub PUT)
};
...
function save(){
  try{
    localStorage.setItem('aushadhi',JSON.stringify({munishri,volunteers,assignment,curWeek,
      periodStart:_periodStart?toISO(_periodStart):null,
      periodEnd:_periodEnd?toISO(_periodEnd):null}));
    localStorage.setItem('aushadhi-cloud',JSON.stringify(_cloudSettings));
  }catch(e){}
}
function load(){try{
  ...
  try{
    const cs=JSON.parse(localStorage.getItem('aushadhi-cloud')||'{}');
    if(cs.rawUrl) _cloudSettings.rawUrl=cs.rawUrl;
    if(cs.apiUrl) _cloudSettings.apiUrl=cs.apiUrl;
    if(cs.pat)    _cloudSettings.pat   =cs.pat;
    if(cs.fileSha)_cloudSettings.fileSha=cs.fileSha;
  }catch(e){}
}
...
// ══════════════════════════════════════════════════════════════
// CLOUD SYNC ENGINE — GitHub raw file as shared database
// ══════════════════════════════════════════════════════════════
...
function cloudStatus(msg, isError){
  ...
  if(el){
    el.textContent = msg;
    el.style.color = isError ? '#dc2626' : (msg.startsWith('✅') ? '#15803d' : '#64748b');
  }
  const hdrEl = document.getElementById('hdr-cloud');
  if(hdrEl){
    hdrEl.textContent = msg;
    hdrEl.style.opacity = '1';
    clearTimeout(hdrEl._t);
    if(msg.startsWith('✅')) hdrEl._t = setTimeout(()=>hdrEl.style.opacity='.6', 3000);
  }
}

function buildDataPayload(){
  return {
    munishri, volunteers, assignment, curWeek,
    periodStart: _periodStart ? toISO(_periodStart) : null,
    periodEnd:   _periodEnd   ? toISO(_periodEnd)   : null,
    _savedAt: new Date().toISOString(),
    _savedBy: _adminName || 'Admin'
  };
}

// ── PULL: Fetch latest data from GitHub raw URL ─────────────────────────────
async function pullFromCloud(){
  const raw = _cloudSettings.rawUrl.trim();
  if(!raw){ cloudStatus('⚠ No cloud URL set. Open ⚙ Settings first.', true); return; }

  cloudStatus('⏳ Fetching latest data…');
  try{
    // Cache-bust to always get fresh data (not CDN cache)
    const url = raw + (raw.includes('?') ? '&' : '?') + '_t=' + Date.now();
    const res  = await fetch(url);
    if(!res.ok) throw new Error('HTTP ' + res.status);
    const data = await res.json();
    applySnapshot(data);
    const ts = data._savedAt
      ? new Date(data._savedAt).toLocaleString('en-IN',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})
      : '';
    cloudStatus('✅ Updated' + (data._savedBy?' by '+data._savedBy:'') + (ts?' · '+ts:''));
  } catch(e){
    cloudStatus('❌ Pull failed: ' + e.message, true);
  }
}

// ── PUSH: Upload data to GitHub via API ─────────────────────────────────────
async function pushToCloud(){
  if(!_isAdmin){ cloudStatus('⚠ Login as admin to push.', true); return; }
  const { apiUrl, pat, fileSha } = _cloudSettings;
  if(!apiUrl || !pat){
    cloudStatus('⚠ GitHub API URL and token not set. Open ⚙ Settings.', true);
    return;
  }

  cloudStatus('⏳ Pushing to cloud…');
  try{
    const payload = buildDataPayload();
    const content = btoa(unescape(encodeURIComponent(JSON.stringify(payload, null, 2))));
    const body    = { message: 'Aushadhi sync ' + new Date().toISOString(), content };
    if(fileSha) body.sha = fileSha; // required for update (not first upload)
    ...
    // Store new SHA for next push
    _cloudSettings.fileSha = json.content && json.content.sha ? json.content.sha : '';
    save(); // persist new SHA
    cloudStatus('✅ Pushed to cloud — all viewers will see this');
  } catch(e){
    cloudStatus('❌ Push failed: ' + e.message, true);
  }
}
...
function showCloudSettings(){
  ...
  document.getElementById('cs-raw').value   = _cloudSettings.rawUrl;
  document.getElementById('cs-api').value   = _cloudSettings.apiUrl;
  document.getElementById('cs-pat').value   = _cloudSettings.pat;
  document.getElementById('cs-name').value  = _adminName;
  document.getElementById('cs-pin').value   = '';
  document.getElementById('cs-error').textContent = '';
}
function hideCloudSettings(){
  document.getElementById('cloud-modal').style.display='none';
}
async function saveCloudSettings(){
  const rawUrl  = document.getElementById('cs-raw').value.trim();
  const apiUrl  = document.getElementById('cs-api').value.trim();
  const pat     = document.getElementById('cs-pat').value.trim();
  const name    = document.getElementById('cs-name').value.trim();
  const pin     = document.getElementById('cs-pin').value.trim();
  const errEl   = document.getElementById('cs-error');

  // Validate raw URL
  if(rawUrl && !rawUrl.startsWith('https://raw.githubusercontent.com/')){
    errEl.textContent = 'Raw URL must start with https://raw.githubusercontent.com/';
    return;
  }

  // Verify PIN if provided (to enable admin mode)
  if(pin){
    const hash = await sha256(pin);
    if(hash !== window._adminPinHash){
  ...
  _cloudSettings.rawUrl = rawUrl;
  _cloudSettings.apiUrl = apiUrl;
  _cloudSettings.pat    = pat;
  if(name && _isAdmin) _adminName = name;
  save();
  hideCloudSettings();
  cloudStatus('✅ Settings saved');
  if(rawUrl) setTimeout(pullFromCloud, 300);
}

// ── Admin mode helpers (reused from online version) ─────────────────────────
...
function setAdminUI(isAdmin){
  ...
  if(badge){ badge.textContent=isAdmin?'🔑 Admin':'👁 Viewer';
    badge.style.background=isAdmin?'#15803d':'#64748b'; }
}
function logoutAdmin(){
  _isAdmin=false; _adminName='';
  setAdminUI(false);
  cloudStatus('Switched to viewer mode');
}

// ── applySnapshot (same as online version) ──────────────────────────────────
function applySnapshot(d){
  ...
  if(d.periodStart){_periodStart=parseLocalDate(d.periodStart)||new Date(d.periodStart+'T12:00:00');}
  if(d.periodEnd)  {_periodEnd  =parseLocalDate(d.periodEnd)  ||new Date(d.periodEnd  +'T12:00:00');}
  try{localStorage.setItem('aushadhi',JSON.stringify(d));}catch(e){}
  updatePeriodUI();
  initMSSlot();
  setTimeout(()=>{loadMSForm();renderMSCards();syncSelects();renderVolList();
    renderAssignResult();renderRoster();renderSummary();calInit('main');},0);
}
...
// Start in viewer mode; auto-pull if cloud URL is configured
setAdminUI(false);
if(_cloudSettings.rawUrl){
  setTimeout(pullFromCloud, 500);
  cloudStatus('⏳ Syncing from cloud…');
} else {
  cloudStatus('⚙ Configure cloud sync in Settings');
}
